         ...
         <img src="./images/${cat.imageLocation}.jpg" width="250" height="250" alt="Card image cap">
             <div class="info">
              <button class="showBtn">${cat.info?'Hide':'Show'} status code</button>
                    <div class="status" style="${styleMap(cat.info?{display:'block'}:
                    {display:'none'})}" id="${cat.id}">
...
const elemenId = event.target.parentNode.querySelector('.status').id
const cat = cats.find(c=>c.id == elemenId)
cat.info =!cat.info;
update();
